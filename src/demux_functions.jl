    	...
    	for template in collect(keys(samples))
        	...
        	if (indexes["fwd_index"],indexes["rev_index"]) in index_tuples
            	...
            	trimmed_seqs = [
                	...
                	N7_suffix*samples[template]["rev_primer"],S5_suffix*samples[template]["fwd_primer"];
                	buffer = 8)
        	...
        	if (indexes["fwd_index"],indexes["rev_index"]) in index_tuples
				...
				try #very occasionally double_primer_trim throws a bounds error, so we catch that error and pass those reads out for examination
					...
					try #very occasionally fast_primer_match throws a bounds error, so we catch that error and pass those reads out for examination
						...
						#keeps = iterative_primer_match([s for (s,p) in index_trimmed], [samples[template]["fwd_primer"]],12,25; tol_one_error=true) .> 0 #primer matching, primers needs to be array
						#match forward primer requiring the entire primer sequence (preferred for samples that have been indexed)
						keeps = fast_primer_match([s for (s,p) in index_trimmed],[samples[template]["fwd_primer"]],tol_one_error=true) .>0 #this sometimes throws bounds error
						seqs_keeping = index_trimmed[keeps]
						final_trim = [primer_trim(s,p,samples[template]["fwd_primer"]) for (s,p) in seqs_keeping]; #now trim off forward primer
						#println("final_trim")
